...
** 1.1.5
- Fix `mapcar` and `window-live-p` errors in `org-linenote-browse`
- Fix a bug where other overlays were removed when deleting a note
- Fix a bug when adding tags within multiline notes
